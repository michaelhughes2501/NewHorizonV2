  ...
  if (active) return (
    <div className="flex flex-col h-[calc(100vh-8rem)] bg-white rounded-2xl border border-slate-200 overflow-hidden">
      <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100 bg-slate-50">
        <button onClick={() => setActive(null)} aria-label="Back to conversations" className="p-2 rounded-xl hover:bg-slate-200 transition-colors">
          <ArrowLeft size={16} />
        </button>
      ...
      <form onSubmit={send} className="px-4 py-3 border-t border-slate-100 flex gap-2 items-center">
        <button type="button" onClick={() => setIsAnon(p => !p)} title={isAnon ? 'Anonymous mode on' : 'Anonymous mode off'}
          aria-label={isAnon ? 'Disable anonymous mode' : 'Enable anonymous mode'} aria-pressed={isAnon}
        ...
        <button type="submit" disabled={sending || !newMsg.trim()} aria-label="Send message"
